...
import { getAdminAnalytics } from "@/services/user-service";
...
export async function GET(req: NextRequest) {
    try {
        ...
        const analyticsData = await getAdminAnalytics(hospitalId);
        return NextResponse.json(analyticsData);

    } catch (error) {
        console.error("Failed to fetch admin analytics data:", error);
