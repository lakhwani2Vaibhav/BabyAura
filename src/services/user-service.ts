...
export const updateDoctor = async (doctorId: string, updates: Partial<{ name: string; specialty: string; status: 'Active' | 'On Leave' }>) => {
    if (!db) await init();
    // Assuming doctorId is a string from the frontend. MongoDB _id can be ObjectId or string.
    // If your _ids are strings, this is fine. If they are ObjectIds, you need to convert it.
    // Let's assume for now they are strings as generated by your generateId function.
